...
We can then add up the values in RoySocLonWordFlagdf to make a table of the frequency of play and work rhetoric, RoySocLonFreqmat. It's important to do it this way because it allows us to choose a random KWIC, so we can get a sense of the context.
...
```{r RoySocLonFreqmat,  eval=FALSE}
  ...
  RoySocLonWordFlagPlaydf <- RoySocLonWordFlagdf[grep("Play-Rhetoric",RoySocLonWordFlagdf$Category),]
      ...
      RoySocLonFreqmat <- matrix(,ncol=10,nrow=1)
      ...
      for (i in 1:length(files)) {
        ...
        temprows <- matrix(,ncol=10,nrow=2)
        colnames(temprows) <- c("Text", "Text_ID","Start_Date","Category","Frequency","Total_Lemma","Normalized_Freq","Sample_KWIC","Avg_Lemma_Perc","End_Date")
        temprows[1:2,1] <- as.character(TempTextName)
        temprows[1:2,2] <- i
        temprows[1:2,3] <- as.character(strsplit(TempDate,"-")[[1]][1])
        ...
        temprows[1:2,10] <- as.character(strsplit(TempDate,"-")[[1]][2])
...
# Visualizing RoySocLonFreqdf BY DATE
      p <- ggplot(RoySocLonFreqdf, aes(y = as.numeric(as.character(Normalized_Freq)), x =                         as.numeric(substr(Start_Date,1,4)), color = Category, label = Sample_KWIC))
...
While this is helpful, we also want a visualization that reminds us that rather than being points, each of these volumes actually covers a range of dates. We can visualize this by using the geom_errorbarh function.

```{r RoySocLonWordFreqmat VisualBar,  eval=FALSE}
    p <- ggplot()      
pg <- geom_errorbarh(data=RoySocLonFreqdf, aes(y = as.numeric(as.character(Normalized_Freq)), x =                         as.numeric(substr(Start_Date,1,4)),xmin=as.numeric(substr(Start_Date,1,4)),xmax=as.numeric(substr(End_Date,1,4)), color = Category, size=1))
      pl <- p + pg + labs(x = "Date", y = "Normalized Frequency (% of Words in Text)", title = "Appearances of Play and Work Rhetoric within Proceedings \nof the Royal Society") + guides(size=FALSE)
    pl
```
